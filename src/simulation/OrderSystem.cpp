...
void OrderSystem::issueOrder(const Order &order)
{
    ...
    std::cout << (order.type == OrderType::MOVE ? "MOVE" : order.type == OrderType::ATTACK ? "ATTACK"
                                                       : order.type == OrderType::DEFEND   ? "DEFEND"
                                                                                           : "HOLD")
              << std::endl;
}
...
void OrderSystem::Simulate(MAP::GRID* map)
{
    movementSystem.Simulate(map);
}
